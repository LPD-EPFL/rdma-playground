...
#define TIMESTAMP_INIT HRT_INITT
#define TIMESTAMP_T HRT_TIMESTAMP_TT
...
#define GET_TIMESTAMP(t) clock_gettime(CLOCK_REALTIME, &t)
#define ELAPSED_NSEC(t1, t2) (t2.tv_nsec + t2.tv_sec * 1000000000UL - t1.tv_nsec - t1.tv_sec * 1000000000UL)
#define TO_NSEC(t) (t.tv_nsec + t.tv_sec * 1000000000UL)
#define IS_ZERO(t) (t.tv_nsec == 0 && t.tv_sec == 0)
#endif
...
#define HRT_TIMESTAMP_TT x86_64_timeval_tt
...
#define HRT_GET_ELAPSED_TICKSS(t1, t2, numptr) \
    *numptr =                                 \
...
#define HRT_INITT                                                   \
    do {                                                                \
        static volatile HRT_TIMESTAMP_TT t1, t2;                         \
        ...
        while (notsmaller < 3) {                                        \
            ...
            sleep(1);                                                   \
            GET_TIMESTAMP(t2);                                          \
            HRT_GET_ELAPSED_TICKSS(t1, t2, &elapsed_ticks);              \
            notsmaller++;                                               \
...
#define HRT_GET_TIMEE(t1, time) time = ((((UINT64_T)t1.h) << 32) | t1.l)
