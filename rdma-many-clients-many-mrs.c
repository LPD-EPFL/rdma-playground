...
int main(int argc, char *argv[])
{
    ...
    if(data.servername){
        ...
        // ibv_rereg_mr(ctx->mr[1], IBV_REREG_MR_CHANGE_ACCESS, ctx->pd, ctx->buf, ctx->size * 2, IBV_ACCESS_LOCAL_WRITE);
        // if (num_clients > 1) {
        //     // give first client right to write
        //     permission_switch(ctx->mr[1], ctx->mr[0], ctx->pd, ctx->buf, ctx->size * 2, IBV_ACCESS_LOCAL_WRITE, IBV_ACCESS_LOCAL_WRITE | IBV_ACCESS_REMOTE_WRITE);
        // }
...
static void set_local_ib_connection(struct app_context *ctx, struct app_data *data){
    ...
    data->local_connection.qpn = malloc(num_clients * sizeof(int));
    for (int i = 0; i < num_clients; ++i) {
        data->local_connection.qpn[i] = ctx->qp[i]->qp_num;
        data->local_connection.rkey = ctx->mr[i]->rkey;
    }
...
static int tcp_exch_ib_connection_info(struct app_data *data){
    ...
    for (int i = 0; i < num_clients; ++i) {
        ...
        sprintf(msg, "%04x:%06x:%06x:%08x:%016Lx", 
                local->lid, local->qpn[i], local->psn, local->rkey, local->vaddr);
        ...
        remote->qpn = malloc(num_clients * sizeof(int));
        parsed = sscanf(msg, "%x:%x:%x:%x:%Lx", 
                            &remote->lid, &remote->qpn[i], &remote->psn, &remote->rkey, &remote->vaddr);
...
static int qp_change_state_rtr(struct ibv_qp *qp, struct app_data *data, int id){
    ...
    attr->dest_qp_num           = data->remote_connection[id].qpn[id];
