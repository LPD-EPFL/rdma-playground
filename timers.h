...
#define ELAPSED_NSEC(t1, t2) t2.tv_nsec-t1.tv_nsec
#endif
...
#define HRT_GET_TIMESTAMP_UNSAFE(t1)  __asm__ __volatile__ ("rdtsc" : "=a" (t1.l), "=d" (t1.h));
#define HRT_GET_TIMESTAMP_SAFE(t1)  __asm__ __volatile__ ("rdtscp" : "=a" (t1.l), "=d" (t1.h));
...
#define HRT_GET_NSEC(ticks) (unsigned long) (1e9*(double)ticks/(double)g_timerfreq)

#define HRT_GET_INTERVAL_NSEC(t1, t2) \
    HRT_GET_NSEC(  (\
    (((( UINT64_T ) t2.h) << 32) | t2.l) - \
    (((( UINT64_T ) t1.h) << 32) | t1.l) \
    ))
