...
class Environment : public ::testing::Environment {
   public:
    ...
    void SetUp() override {
        ...
        printf(
            "PID=%d | port=%d | ib_port=%d | size=%lu | tx_depth=%d | sl=%d "
            "|\n",
    ...
    void TearDown() override { consensus_shutdown(); }
};
...
TEST(RDMATest, UnexpectedError) {
    ...
    if (g_ctx.my_index == 2) {
        ...
        TEST_NZ(ibv_rereg_mr(g_ctx.qps[0].mr_write, IBV_REREG_MR_CHANGE_ACCESS,
...
TEST(RDMATest, BigTest) {
    ...
    if (g_ctx.my_index == 2) {
        ...
        TEST_NZ(ibv_rereg_mr(g_ctx.qps[0].mr_write,       // the memory region
                             IBV_REREG_MR_CHANGE_ACCESS,  // we want to change
                                                          // the access flags
    ...
    //     log_write_local_slot_string(g_ctx.buf.log,
    //     g_ctx.buf.log->firstUndecidedOffset, 4, "blablabla");
    //     log_increment_fuo(g_ctx.buf.log);
    //     log_write_local_slot_uint64(g_ctx.buf.log,
    //     g_ctx.buf.log->firstUndecidedOffset, 4, 5);
    //     log_increment_fuo(g_ctx.buf.log);
    //     log_write_local_slot_uint64(g_ctx.buf.log,
    //     g_ctx.buf.log->firstUndecidedOffset, 4, 5);
    ...
    // // output latency
    // uint64_t duration = (end.tv_sec * 1000000 + end.tv_usec) - (start.tv_sec
    // * 1000000 + start.tv_usec ); double avg_latency = (1.0 * duration) /
    // NB_ITERATIONS;
    ...
    //     // For now, the message to be written into the clients buffer can be
    //     edited here
    ...
    //     // for (int i = 0; i < g_ctx.num_clients; ++i) {
    //     //     // rdma_write(i);
    //     //     uint64_t remote_addr = log_get_remote_address(g_ctx.buf.log,
    //     slot, ((log_t*)g_ctx.qps[i].remote_connection.vaddr));
    //     //     post_send(g_ctx.qps[i].qp, slot, sizeof(log_slot_t),
    //     g_ctx.qps[i].mr->lkey, g_ctx.qps[i].remote_connection.rkey,
    //     remote_addr, IBV_WR_RDMA_WRITE, 42);

    //     // }

    //     // struct ibv_wc wc;
    //     // //int n, uint64_t round_nb, struct ibv_cq *cq, int num_entries,
    ...
    //     // permission_switch(g_ctx.mr[1], g_ctx.mr[0], g_ctx.pd, g_ctx.buf,
    //     g_ctx.size*2, IBV_ACCESS_LOCAL_WRITE, IBV_ACCESS_REMOTE_WRITE |
    //     IBV_ACCESS_LOCAL_WRITE);
    //     // permission_switch(g_ctx.mr[0], g_ctx.mr[1], g_ctx.pd, g_ctx.buf,
    //     g_ctx.size*2, IBV_ACCESS_LOCAL_WRITE, IBV_ACCESS_LOCAL_WRITE |
    //     IBV_ACCESS_REMOTE_WRITE);
    //     // ibv_rereg_mr(g_ctx.mr[0], IBV_REREG_MR_CHANGE_ACCESS, g_ctx.pd,
    //     g_ctx.buf, g_ctx.size * 2, IBV_ACCESS_LOCAL_WRITE);
    //     // ibv_rereg_mr(g_ctx.mr[1], IBV_REREG_MR_CHANGE_ACCESS, g_ctx.pd,
    //     g_ctx.buf, g_ctx.size * 2, IBV_ACCESS_LOCAL_WRITE |
    //     IBV_ACCESS_REMOTE_WRITE);

    //     sleep(3);
    //     printf("Client. Reading Local-Buffer (Buffer that was registered with
    //     MR)\n");
