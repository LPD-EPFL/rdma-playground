...
ssh "$USER@${nodes[$j]}" /bin/bash << EOF
    ...
    $start_memcached
    tmux send-keys -t peregrin:main "CONFIG=../config/helvetios/${#nodes[@]}/config.${nodes[$j]}.toml $LEADER ../memcached/target/bin/memcached -l 0.0.0.0 -p 11222" C-m
EOF
...
stopme() {
    for ((j=0; j<${#nodes[@]}; j++)); do
        ssh "$USER@${nodes[$j]}" "pkill -9 memcached; killall memcached; pkill -9 memcached; killall memcached; tmux kill-session -t peregrin"
    done
}
